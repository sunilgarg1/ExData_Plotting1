...
# Combined Plot
# No specific instruction on whether the plots need to be arranged by rows or columns, so just arranging by rows
png(filename = "plot4.png", width = 480, height = 480)
par(mfrow = c(2, 2), mar = c(5, 4, 4, 2), oma = c(0, 0, 0, 0))
with(electricConsumption_Df, plot(Date_Time,Global_active_power, type="l", xlab = "", ylab = "Global Active Power"))
with(electricConsumption_Df, plot(Date_Time,Voltage, type="l", xlab = "datetime", ylab = "Voltage"))
...
with(electricConsumption_Df, plot(Date_Time,Global_reactive_power, type="l", xlab = "datetime", ylab = "Global_reactive_power"))
dev.off()
